...
test("selects explicit, hosted, local, and test database names", () => {
	expect(databaseName({ MONGODB_DATABASE: "ds9-operations" })).toBe("ds9-operations");
	expect(databaseName({ NODE_ENV: "production" })).toBe("command-center-ai-production");
	expect(databaseName({ RAILWAY_ENVIRONMENT_NAME: "Review / 42" })).toBe("command-center-ai-production");
	expect(
		databaseName({
			NODE_ENV: "production",
	...
	expect(databaseName({ NODE_ENV: "test" })).toMatch(/^command-center-ai-test-/);
	expect(databaseName({ USER: "Benjamin Sisko" })).toBe("command-center-ai-local-benjamin-sisko");
});
