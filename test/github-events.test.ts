...
test("GitHub verifies, dedupes, fans out, and clears successful deliveries", () => withDatabase(async (db) => {
  await upsertIdentity(db, "u1", "sisko"); await upsertIdentity(db, "u2", "kira"); await bindInstallation(db, "u1", "9", "cubanx"); await bindInstallation(db, "u2", "9", "cubanx");
...
test("closed pull requests remove their projection", () => withDatabase(async (db) => {
  await upsertIdentity(db, "u", "sisko"); await bindInstallation(db, "u", "9", "cubanx"); await acceptGitHubDelivery(db, "open", "pull_request", body); await drainInbox(db);
...
test("webhook retries retain payload and bot plus OpenSpec updates preserve formal review", () => withDatabase(async (db) => {
  await upsertIdentity(db, "u", "sisko"); await bindInstallation(db, "u", "9", "cubanx"); await acceptGitHubDelivery(db, "pr", "pull_request", body); await drainInbox(db);
  const comment = JSON.stringify({ action: "created", installation: { id: 9, account: { login: "cubanx" } }, repository: { id: 2 }, issue: { number: 7, pull_request: {} }, comment: { user: { login: "Claude[bot]" }, body: "started review" } });
...
test("webhook branches reject whitespace, overlong, and dotdot refs", () => withDatabase(async (db) => {
  ...
  for (const [index, ref] of ["ops/ bad", "x".repeat(256), "ops/../bad"].entries()) await acceptGitHubDelivery(db, `push-branch-${index}`, "push", JSON.stringify({ installation: { id: 9, account: { login: "cubanx" } }, repository: { id: 2 }, ref: `refs/heads/${ref}`, after: "a".repeat(40), commits: [{ modified: [`openspec/changes/branch-${index}/tasks.md`] }] }));
...
test("event notifications are user-scoped and transition-deduplicated", () => withDatabase(async (db) => {
  await upsertIdentity(db, "9", "sisko"); await upsertIdentity(db, "10", "kira"); await bindInstallation(db, "9", "1", "cubanx");
