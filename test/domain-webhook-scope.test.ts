...
test("webhook projection preserves evidence written after its source read", () =>
	withDatabase(async (db) => {
		const { upsertPullRequest } = await import("#/db");
		await upsertIdentity(db, "1701", "sisko");
		await bindInstallation(db, "1701", "42", "cubanx");
		...
		const find = db.pullRequests.find.bind(db.pullRequests);
		vi.spyOn(db.pullRequests, "find").mockImplementationOnce((...args) => {
			const cursor = find(...args),
				read = cursor.toArray.bind(cursor);
			cursor.toArray = async () => {
				const rows = await read();
				await upsertPullRequest(db, { repositoryId: "301", number: 7, review_state: "approved" });
				return rows;
			};
			return cursor;
		});
		...
		await drainInbox(db);
		expect(await db.pullRequests.findOne({ _id: "301:7" })).toMatchObject({
			title: "Shields restored",
