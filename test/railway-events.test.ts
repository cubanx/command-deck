...
test("Railway hints require a complete shape and an authoritative match before state", async () => {
  ...
  expect(db.query("SELECT verification_state FROM deployments WHERE id='d'").get()!.verification_state).toBe("pending");
  ...
  acceptRailwayHint(db, "error", '{"projectId":"p","serviceId":"s","environmentId":"e","deploymentId":"broken"}');
  await drainInbox(db, async () => { throw new Error("station offline"); });
  expect(db.query("SELECT verification_state FROM deployments WHERE id='broken'").get()!.verification_state).toBe("error");
});
