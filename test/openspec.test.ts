...
test("projects installation-scoped OpenSpec progress", () => withDatabase(async (db) => {
  ...
  expect(await projectOpenSpec(db, { installationId: "1", accountLogin: "cubanx", repositoryId: "r", path: "openspec/changes/defiant/tasks.md", content: "- [x] Ready", sha: "a".repeat(40) })).toBeTrue();
  expect((await db.users.findOne({ _id: "u" }))?.installations[0]?.repositories[0]?.openSpecs).toHaveLength(1);
  expect(await projectOpenSpec(db, { installationId: "1", accountLogin: "cubanx", repositoryId: "r", path: "openspec/changes/defiant/tasks.md", content: "- [x] Ready", sha: "a".repeat(40), sourceRef: "main" })).toBeFalse();
  await projectOpenSpec(db, { installationId: "1", accountLogin: "cubanx", repositoryId: "r", path: "openspec/changes/defiant/tasks.md", deleted: true, sha: "b".repeat(40) });
