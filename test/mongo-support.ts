import { closeDatabase, type Db, initializeDatabase, openDatabase, testDatabaseGuard } from "#/db";
	...
	try {
		...
		try {
			...
			if (!(error instanceof Error) || !error.message.includes("Client must be connected"))
				console.error("MongoDB test cleanup failed", error instanceof Error ? error.message : "unknown error");
		}
...
export const testConfig = {
	...
	mongoDatabase: "command-center-ai-test",
	localDemo: false,
	production: false,
	secureCookies: true,
};
