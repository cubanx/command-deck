	...
	withDatabase(async (db) => {
		...
		await mutateUser(db, "u", (user) => {
			user.installations[0]!.repositories.push({
				...
				openSpecs: [],
				pullRequests: [
					{ number: 7, author_login: "kira", state: "open", head_sha: sha, updated_at: "2026-09-09T12:00:00Z" },
				],
			});
		});
					...
					await acceptGitHubDelivery(
						...
						JSON.stringify({
							...
							pull_request: {
								...
								user: { login: "kira" },
								head: { sha: state === "NEW_HEAD" ? "b".repeat(40) : sha },
		...
		const repo = (await db.users.findOne({ _id: "u" }))!.installations[0]!.repositories[0]!;
		expect(repo.pullRequests).toMatchObject([
			state === "NEW_HEAD"
				? { number: 7, state: "open", head_sha: "b".repeat(40) }
				: { number: 7, state: "closed", merged: true, retention_candidate: true },
		]);
		expect(repo.openSpecs).toMatchObject([
...
test.each([false, true])("late merged evidence cannot undo newer completion=%s", (newerComplete) =>
	withDatabase(async (db) => {
				...
				pullRequests: [
					{
						...
						post_merge_obligations: ["defiant", "bajor"],
					},
				],
			});
		});
		...
		const original = (await db.users.findOne({ _id: "u" }))!.installations[0]!.repositories[0]!;
		await mutateUser(db, "alias", (user) => {
			const repository = structuredClone(original);
			repository.pullRequests[0]!.post_merge_obligations = ["wormhole"];
			user.installations[0]!.repositories.push(repository);
		});
		const connection = { nodes: [], pageInfo: { hasNextPage: false } };
		...
		for (const userId of ["u", "alias"]) {
			const repo = (await db.users.findOne({ _id: userId }))!.installations[0]!.repositories[0]!;
			expect(repo.pullRequests).toHaveLength(newerComplete ? 0 : 1);
			if (!newerComplete)
				expect(repo.pullRequests[0]).toMatchObject({
					post_merge_source_commit: newerSha,
					post_merge_obligations: ["bajor", "defiant", "wormhole"],
				});
			expect(repo.openSpecs).toHaveLength(3);
			for (const spec of repo.openSpecs)
	...
	withDatabase(async (db) => {
		...
		await mutateUser(db, "u", (user) => {
			user.installations[0]!.repositories.push({
				...
				deployments: [],
				openSpecs: [],
				pullRequests: [{ number: 7, author_login: "kira", state: "open", head_sha: "a".repeat(40) }],
			});
		});
		await bootstrapInstallation(
			...
			async (url) => {
				...
				if (value.includes("/pulls?")) {
					await mutateUser(db, "u", (user) => {
						const repo = user.installations[0]!.repositories[0]!;
						repo.pullRequests = complete
							? []
							: [{ number: 7, author_login: "kira", state: "closed", merged: true, retention_candidate: true }];
					});
					await projectOpenSpec(db, {
						installationId: "1",
						accountLogin: "cubanx",
		...
		const repo = (await db.users.findOne({ _id: "u" }))!.installations[0]!.repositories[0]!;
		expect(repo.pullRequests).toHaveLength(complete ? 0 : 1);
		if (!complete) expect(repo.pullRequests[0]).toMatchObject({ state: "closed", merged: true });
		expect(repo.openSpecs).toMatchObject([{ change_name: "defiant", source_commit: "b".repeat(40) }]);
	}),
);
