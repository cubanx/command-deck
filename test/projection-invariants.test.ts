...
const base = { installation: { id: 1 }, repository: { id: 2, full_name: "ds9/ops" } };
test("GitHub projections close PRs and dedupe review, check, and mergeability transitions", async () => {
  const db = openDatabase(); db.query("INSERT INTO users (id,github_id,login) VALUES ('u','42','kira')").run(); bindInstallation(db, "u", "1");
...
test("push projects only changed committed task artifacts and completion once", async () => {
  ...
  const push = (sha: string, content: string) => JSON.stringify({ ...base, after: sha, commits: [{ modified: ["openspec/changes/defiant/tasks.md", "README.md"] }] });
