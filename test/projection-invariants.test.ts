...
test("provider identity mutations are idempotent", () => withDatabase(async (db) => {
  await upsertIdentity(db, "u", "kira"); await bindInstallation(db, "u", "1");
  const body = JSON.stringify({ installation: { id: 1 }, repository: { id: 2, full_name: "ds9/ops" }, pull_request: { number: 7, title: "Defend", user: { login: "kira" }, state: "open" } });
