...
const mergedPullRequest = (number: number, head = sha("a"), merge = sha("b")) => ({
	action: "closed",
...
test("merged evidence fails closed, expires, caps, and leaves uncorrelated deployments blank", () =>
	withDatabase(async (db) => {
		...
		await acceptGitHubDelivery(db, "invalid-merge", "pull_request", JSON.stringify(invalid));
...
test("newest deployment status survives unordered bootstrap and a stale webhook", () =>
	withDatabase(async (db) => {
		...
		expect((await dashboardForUser(db, "u", new Date("2030-01-03"))).deployments[0]).toMatchObject({
			id: "7",
			state: "success",
			status_id: "101",
...
test("deployment status is monotonic, bounded, and retains safe links", () =>
	withDatabase(async (db) => {
		await upsertIdentity(db, "u", "sisko");
		await bindInstallation(db, "u", "1", "cubanx");
		const delivery = (deliveryId: string, id: number, updated: string, state = "success") =>
