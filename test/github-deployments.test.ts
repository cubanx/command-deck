...
test("deployment events remain installation-scoped", () => withDatabase(async (db) => {
  ...
  const body = JSON.stringify({ installation: { id: 1 }, repository: { id: 2, full_name: "ds9/ops" }, deployment: { id: 7, environment: "production", ref: "main", sha: "a".repeat(40), created_at: new Date().toISOString() }, deployment_status: { state: "success", created_at: new Date().toISOString() } });
...
test("deployment status is monotonic, bounded, and retains safe links", () => withDatabase(async (db) => {
  await upsertIdentity(db, "u", "sisko"); await bindInstallation(db, "u", "1");
  const delivery = (deliveryId: string, id: number, updated: string, state = "success") => acceptGitHubDelivery(db, deliveryId, "deployment_status", JSON.stringify({ installation: { id: 1 }, repository: { id: 2, full_name: "ds9/ops" }, deployment: { id, created_at: updated }, deployment_status: { state, created_at: updated, target_url: "https://example.test/deploy", log_url: "https://example.test/log" } }));
