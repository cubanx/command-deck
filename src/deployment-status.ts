...
const time = (status: Status) =>
	Date.parse(
		String(
			status.status_created_at ?? status.created_at ?? status.updated_at ?? "",
		),
	);
const statusId = (status: Status) =>
	typeof status.status_id === "number" || typeof status.status_id === "string"
...
export function latestDeploymentStatus(statuses: Status[]) {
	return statuses.reduce<Status | undefined>(
		(latest, status) =>
			!latest || compareDeploymentStatus(status, latest) > 0 ? status : latest,
		undefined,
	);
}
