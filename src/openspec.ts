...
export async function projectOpenSpec(db: Db, input: { installationId: string; accountLogin: string; repositoryId: string; path: string; content?: string; deleted?: boolean; sha: string; sourceRef?: string }) {
  const changeName = input.path.split("/")[2];
  if (!changeName) throw new Error("invalid OpenSpec tasks path");
  if (input.deleted) { const users = await db.users.find({ "installations.installationId": input.installationId }, { projection: { _id: 1 } }).toArray(); await Promise.all(users.map((user) => mutateUser(db, user._id, (aggregate) => { const installation = aggregate.installations.find((item) => item.installationId === input.installationId); if (!installation || !approvedInstallationAccount(installation.accountLogin) || !sameLogin(installation.accountLogin, input.accountLogin)) return; const repository = installation.repositories.find((item) => item.repositoryId === input.repositoryId); if (repository) repository.openSpecs = repository.openSpecs.filter((item) => item.change_name !== changeName); }))); return false; }
  ...
  await Promise.all(users.map((user) => mutateUser(db, user._id, (aggregate) => { const installation = aggregate.installations.find((item) => item.installationId === input.installationId); if (!installation || !approvedInstallationAccount(installation.accountLogin) || !sameLogin(installation.accountLogin, input.accountLogin)) return; const repository = installation.repositories.find((item) => item.repositoryId === input.repositoryId); if (!repository) return; const index = repository.openSpecs.findIndex((item) => item.change_name === changeName), previous = index >= 0 ? repository.openSpecs[index] : undefined; completed ||= progress.total > 0 && progress.completed === progress.total && (!previous || Number(previous.completed) < Number(previous.total)); const next = { change_name: changeName, completed: progress.completed, total: progress.total, source_commit: input.sha, ...(input.sourceRef ? { source_ref: input.sourceRef } : {}), active_group: progress.activeGroup ? JSON.stringify(progress.activeGroup) : null, updated_at: new Date().toISOString() }; if (index >= 0) repository.openSpecs[index] = next; else repository.openSpecs.push(next); })));
  return completed;
}
