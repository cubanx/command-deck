...
export async function dashboardForUser(db: Db, userId: string, now = new Date()) {
  ...
  const openPullRequests = projectedPullRequests.filter((pr) => pr.state === "open");
  ...
  return { pullRequests, deployments, notifications: notifications.map((notification) => ({ ...notification, id: notification._id })), installationCount: user.installations.length, stale: user.installations.some((installation) => Boolean(installation.lastSyncError)) };
}
