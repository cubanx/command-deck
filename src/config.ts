...
const integer = (
	...
	if (!Number.isInteger(candidate) || !valid(candidate))
		throw new Error(message);
	return candidate;
};

const localDemoConfig = (env: Record<string, string | undefined>) => {
	if (env.DCC_LOCAL_DEMO && !["0", "1"].includes(env.DCC_LOCAL_DEMO))
		throw new Error("DCC_LOCAL_DEMO must be 0 or 1");
...
const reviewBotConfig = (
	env: Record<string, string | undefined>,
): ReviewBotConfig | undefined => {
	const values = [
		env.GITHUB_REVIEW_BOT_LOGIN,
		env.GITHUB_REVIEW_BOT_START_MARKER,
		env.GITHUB_REVIEW_BOT_DONE_MARKER,
	];
	...
	if (
		login &&
		(!/^[A-Za-z0-9][A-Za-z0-9_\-[\]]*$/.test(login) || login.length > 100)
	)
		throw new Error("review bot login is invalid");
	if (
		(startMarker && startMarker.length > 200) ||
		(doneMarker && doneMarker.length > 200)
	)
		throw new Error("review bot markers must be at most 200 characters");
	return login && startMarker && doneMarker
		? { login, startMarker, doneMarker }
		: undefined;
};
...
const productionUrls = (
	...
	if (!production)
		return {} as { publicUrl?: string; oauthCallbackUrl?: string };
	const required = [
		"PUBLIC_URL",
		"MONGODB_URI_BASE",
		"MONGODB_DATABASE",
	...
	for (const name of required) {
		...
		if (
			!value ||
			/^(changeme|replace|example|placeholder|your)([-_\s]|$)/i.test(value)
		)
	...
	if (
		...
		throw new Error(
			"PUBLIC_URL must match RAILWAY_PUBLIC_DOMAIN as an HTTPS origin",
		);
	return {
...
export function loadConfig(
	...
	const hosted = production || Boolean(env.RAILWAY_ENVIRONMENT_NAME);
	const mongoDatabase =
	...
	if (hosted && mongoDatabase !== "command-center-ai-production")
