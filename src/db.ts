...
export function databaseName(
	...
	if (env.NODE_ENV === "production") return "command-center-production";
	if (env.RAILWAY_ENVIRONMENT_NAME)
		return `command-center-ai-${env.RAILWAY_ENVIRONMENT_NAME.replace(/[^a-z0-9-]/gi, "-").toLowerCase()}`;
	if (env.NODE_ENV === "test")
		return `command-center-test-${crypto.randomUUID()}`;
	return `command-center-local-${(env.USER ?? "local").replace(/[^a-z0-9-]/gi, "-").toLowerCase()}`;
}
export function mongoConfig(
	...
	if (env.NODE_ENV === "production" && database !== "command-center-production")
...
export function testDatabaseGuard(database: string) {
	if (!/^command-center-test-[a-f0-9-]{36}$/i.test(database))
		throw new Error(
			"MONGODB_DATABASE must be an explicitly isolated non-production command-center-test UUID database",
		);
}
