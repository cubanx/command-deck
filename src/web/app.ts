...
export const localSpecsFor = (pr: PullRequest, pullRequests: PullRequest[], specs = localSpecs) => {
	...
	if (commitMatches.length) return orderedOpenSpecs(commitMatches);
	...
	return uniqueBranch ? orderedOpenSpecs(branchMatches) : [];
};
...
const render = (x: DashboardSnapshot | null) => {
	...
	const allPullRequests = x.pullRequests.map((pr) => {
		...
		const specs = pr.open_specs ?? (pr.open_spec ? [pr.open_spec] : []);
		return {
	...
	const dashboardMarkup =
		...
		rows(prs, "No open authored pull requests.", (item) => {
			...
			return (
				...
				"</div>" +
				openSpecMarkup(pr.open_specs ?? (item.spec ? [item.spec] : [])) +
				(item.localSpecs?.length
					? `<section aria-label="Local OpenSpec evidence"><p class="muted">Local detected OpenSpec evidence</p>${openSpecMarkup(item.localSpecs)}</section>`
					: "") +
