...
export const buildBrowserScript = async ({ write = false } = {}) => {
	if (typeof Bun === "undefined") return readFile(new URL("../../dist/app.js", import.meta.url), "utf8");
	...
	if (!result.success || !output) throw new Error("Browser TypeScript build failed");
	return output.text();
};
