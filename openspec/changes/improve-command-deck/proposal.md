...
- `command-center-dashboard`: Compact OpenSpec evidence, accessible heading removal, unified configuration entry points, repository-specific checkout state, reconciliation feedback, and merge affordances.
...
- Affects the dashboard/server routes, browser-local configuration and File System Access flow, GitHub client and event projection, authentication/authorization checks, styles, and focused behavioral/accessibility tests.
