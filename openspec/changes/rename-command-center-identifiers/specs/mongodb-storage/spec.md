...
- **THEN** it accepts only `command-center-production` as the configured MongoDB database

#### Scenario: Destructive test database is isolated
- **WHEN** a destructive MongoDB test is prepared
- **THEN** its database name starts with `command-center-test-`
