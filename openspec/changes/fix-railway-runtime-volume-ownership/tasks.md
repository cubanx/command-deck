## 1. Container Validation Contract

- [x] 1.1 Update the dependency-free Docker volume validation test to require intrinsic root initialization without `RAILWAY_RUN_UID=0`, then assert PID 1 runs as UID 1000 and SQLite/WAL files are owned by UID/GID 1000 on a fresh root-owned `/data` volume.
...
- [x] 3.1 Run the Docker build/runtime validation test, existing Bun tests, typecheck, POSIX shell syntax checks, and `git diff --check`.
